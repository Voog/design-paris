:root {
  /* VoogStyle
    "pathI18n": ["product", "title"],
    "titleI18n": "size",
    "editor": "rangePicker",
    "min": 8,
    "max": 100,
    "unit": "px",
    "scope": "global"
  */
  --product-title-size: 29px;
  /* VoogStyle
    "pathI18n": ["product", "title"],
    "titleI18n": "font_size",
    "type": "button",
    "editor": "toggleIcon",
    "states": {
      "on": "700",
      "off": "400"
    },
    "icon": "bold",
    "scope": "global"
  */
  --product-title-font-weight: 700;
  /* VoogStyle
    "pathI18n": ["product", "title"],
    "titleI18n": "font_style",
    "type": "button",
    "editor": "toggleIcon",
    "states": {
      "on": "italic",
      "off": "normal"
    },
    "icon": "italic",
    "scope": "global"
  */
  --product-title-font-style: normal;
  /* VoogStyle
    "pathI18n": ["product", "title"],
    "titleI18n": "text_decoration",
    "type": "button",
    "editor": "toggleIcon",
    "states": {
      "on": "underline",
      "off": "none"
    },
    "icon": "underline",
    "scope": "global"
  */
  --product-title-font-text-decoration: none;
  /* VoogStyle
    "pathI18n": ["product", "title"],
    "titleI18n": "color",
    "type": "button",
    "editor": "colorPicker",
    "featured": true,
    "scope": "global"
  */
  --product-title-color: white;
  /* VoogStyle
    "pathI18n": ["product", "price"],
    "titleI18n": "size",
    "editor": "rangePicker",
    "min": 8,
    "max": 100,
    "unit": "px",
    "scope": "global"
  */
  --product-price-size: 24px;
  /* VoogStyle
    "pathI18n": ["product", "price"],
    "titleI18n": "font_size",
    "type": "button",
    "editor": "toggleIcon",
    "states": {
      "on": "700",
      "off": "400"
    },
    "icon": "bold",
    "scope": "global"
  */
  --product-price-font-weight: 700;
  /* VoogStyle
    "pathI18n": ["product", "price"],
    "titleI18n": "font_style",
    "type": "button",
    "editor": "toggleIcon",
    "states": {
      "on": "italic",
      "off": "normal"
    },
    "icon": "italic",
    "scope": "global"
  */
  --product-price-font-style: normal;
  /* VoogStyle
    "pathI18n": ["product", "price"],
    "titleI18n": "text_decoration",
    "type": "button",
    "editor": "toggleIcon",
    "states": {
      "on": "underline",
      "off": "none"
    },
    "icon": "underline",
    "scope": "global"
  */
  --product-price-font-text-decoration: none;
  /* VoogStyle
    "pathI18n": ["product", "price"],
    "titleI18n": "color",
    "type": "button",
    "editor": "colorPicker",
    "featured": true,
    "scope": "global"
  */
  --product-price-color: white;
  /* VoogStyle
    "pathI18n": ["product", "description"],
    "titleI18n": "size",
    "editor": "rangePicker",
    "min": 8,
    "max": 100,
    "unit": "px",
    "scope": "global"
  */
  --product-description-size: 18px;
  /* VoogStyle
    "pathI18n": ["product", "description"],
    "titleI18n": "font_size",
    "type": "button",
    "editor": "toggleIcon",
    "states": {
      "on": "700",
      "off": "400"
    },
    "icon": "bold",
    "scope": "global"
  */
  --product-description-font-weight: 400;
  /* VoogStyle
    "pathI18n": ["product", "description"],
    "titleI18n": "font_style",
    "type": "button",
    "editor": "toggleIcon",
    "states": {
      "on": "italic",
      "off": "normal"
    },
    "icon": "italic",
    "scope": "global"
  */
  --product-description-font-style: normal;
  /* VoogStyle
    "pathI18n": ["product", "description"],
    "titleI18n": "text_decoration",
    "type": "button",
    "editor": "toggleIcon",
    "states": {
      "on": "underline",
      "off": "none"
    },
    "icon": "underline",
    "scope": "global"
  */
  --product-description-font-text-decoration: none;
  /* VoogStyle
    "pathI18n": ["product", "description"],
    "titleI18n": "color",
    "type": "button",
    "editor": "colorPicker",
    "featured": true,
    "scope": "global"
  */
  --product-description-color: #8D9091;
}